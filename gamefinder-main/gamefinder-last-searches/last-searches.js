...
const ATTR_LAST_SEARCHES = window.localStorage.getItem(ATTR_AT) + 'LASTSEARCHES';

window.onload = function(){
    container = document.getElementById("container");

    if (window.localStorage.getItem(ATTR_LAST_SEARCHES)) {
        const savedSearches = JSON.parse(window.localStorage.getItem(ATTR_LAST_SEARCHES));
        buildSavedSearches(savedSearches);
    }
};

function buildSavedSearches(savedSearch) {
    if (savedSearch) {
        savedSearch.forEach(gamesFoundInSearch => {
            if (gamesFoundInSearch) {
                gamesFoundInSearch.forEach(foundGame => {
                    container.append(getHtmlRenderedGameSmallCard(foundGame, gameRanking));
                    container.append(getHtmlRenderedGameBigCard(foundGame, gameRanking));
                });
            }
        });
    }
